...
set.seed(42)  # for reproducibility; change to explore stochastic variation

# One timestep of the ABM
step_lv <- function(pop, p_birth, p_eat, p_death) {
  prey     <- filter(pop, type == "prey")
  predators <- filter(pop, type == "predator")

  # Rule 1: prey reproduction
  n_births <- rbinom(1, nrow(prey), p_birth)
  new_prey <- tibble(id   = max(pop$id) + seq_len(n_births),
                     type = "prey")

  # Rule 2: predation via random pairing
  n_pairs  <- min(nrow(predators), nrow(prey))
  if (n_pairs > 0) {
    prey_idx <- sample(nrow(prey), n_pairs, replace = FALSE)
    eaten    <- rbinom(n_pairs, 1, p_eat)
    prey     <- prey[-prey_idx[eaten == 1], ]
    n_new_pred <- sum(eaten)
    new_pred <- tibble(id   = max(pop$id) + nrow(new_prey) + seq_len(n_new_pred),
                       type = "predator")
  } else {
    new_pred <- tibble(id = integer(), type = character())
  }

  # Rule 3: predator death
  pred_survive <- predators[rbinom(nrow(predators), 1, 1 - p_death) == 1, ]

  bind_rows(prey, pred_survive, new_prey, new_pred)
}
```

???
Speaker notes: The code structure mirrors the verbal rules exactly. `rbinom` calls make the stochasticity explicit — each event is a Bernoulli trial. The random pairing in Rule 2 is the mean-field assumption in code: `sample(nrow(prey), n_pairs, replace = FALSE)` draws uniformly from the full prey population, giving every predator equal access to every prey. This line is the functional form $\beta NP$ written procedurally.

---

```{r abm-lv-run, echo=TRUE}
# Run ABM for T timesteps, multiple replications
run_lv_abm <- function(T = 300, n_prey0 = 60, n_pred0 = 15,
                       p_birth = 0.06, p_eat = 0.04, p_death = 0.08,
                       seed_val = 42) {
  set.seed(seed_val)
  pop <- tibble(
    id   = seq_len(n_prey0 + n_pred0),
    type = c(rep("prey", n_prey0), rep("predator", n_pred0))
  )
  counts <- tibble(time = 0L,
                   prey = n_prey0, predator = n_pred0)
  for (t in seq_len(T)) {
    if (nrow(filter(pop, type == "prey")) == 0 ||
        nrow(filter(pop, type == "predator")) == 0) break
    pop <- step_lv(pop, p_birth, p_eat, p_death)
    counts <- add_row(counts, time = t,
...
small_runs <- map_dfr(1:20, ~ run_lv_abm(
  ...
  p_birth = 0.06, p_eat = 0.05, p_death = 0.08,
  seed_val = .x
) |> mutate(run = .x))
...
# Agent-based SIR: random mixing version
# Each agent: id, status (S/I/R), days_infected
run_abm_sir <- function(N = 500, I0 = 1, beta = 0.3, gamma = 0.1,
                        T = 150, seed_val = 123) {
  set.seed(seed_val)
  agents <- tibble(
    id             = seq_len(N),
    status         = c(rep("I", I0), rep("S", N - I0)),
    days_infected  = c(rep(1L, I0), rep(0L, N - I0))
  )
  results <- tibble(time = 0L,
                    S = N - I0, I = I0, R = 0L)

  for (t in seq_len(T)) {
    infectious <- filter(agents, status == "I")
    susceptible <- filter(agents, status == "S")

    # Each infectious agent contacts a random susceptible
    if (nrow(infectious) > 0 && nrow(susceptible) > 0) {
      contacts  <- sample(susceptible$id,
                          min(nrow(infectious), nrow(susceptible)),
                          replace = FALSE)
      transmit  <- rbinom(length(contacts), 1, beta)
      new_I_ids <- contacts[transmit == 1]
      agents <- agents |>
        mutate(status = case_when(
          id %in% new_I_ids ~ "I",
          TRUE              ~ status
        ),
        days_infected = if_else(status == "I", days_infected + 1L, days_infected))
    }
    # Recovery
    agents <- agents |>
      mutate(status = case_when(
        status == "I" & rbinom(n(), 1, gamma) == 1 ~ "R",
        TRUE                                        ~ status
      ))
    results <- add_row(results, time = t,
                       S = sum(agents$status == "S"),
                       I = sum(agents$status == "I"),
                       R = sum(agents$status == "R"))
  }
  results
}
...
run_network_sir <- function(graph, N, beta = 0.15, gamma = 0.1,
  ...
  set.seed(seed_val)
  status <- rep("S", N)
  # Infect one random node at t=0
  status[sample(N, 1)] <- "I"

  adj <- as_adjacency_list(graph)
  results <- tibble(time = 0L,
                    S = sum(status == "S"),
                    I = sum(status == "I"),
                    R = sum(status == "R"))
  ...
  for (t in seq_len(T)) {
    ...
    for (i in which(status == "I")) {
      # Transmission along edges only
      neighbors_i <- adj[[i]]
      suscept_nb  <- neighbors_i[status[neighbors_i] == "S"]
      if (length(suscept_nb) > 0) {
        transmit <- rbinom(length(suscept_nb), 1, beta)
        new_status[suscept_nb[transmit == 1]] <- "I"
      }
      # Recovery
      if (rbinom(1, 1, gamma) == 1) new_status[i] <- "R"
    }
    status <- new_status
    results <- add_row(results, time = t,
                       S = sum(status == "S"),
...
# Run on both network types, multiple replications
er_sir_runs <- map_dfr(1:10,
  ~ run_network_sir(er_graph, N_net, seed_val = .x) |> mutate(network = "Erdős-Rényi", run = .x))
ba_sir_runs <- map_dfr(1:10,
  ~ run_network_sir(ba_graph, N_net, seed_val = .x) |> mutate(network = "Barabási-Albert", run = .x))
...
# Spatial Lotka-Volterra: grid-based ABM with local interaction
# Full code in code/ch02_abm_contrast.R
# This chunk generates the gganimate animation shown in the live session

set.seed(314)
GRID  <- 40          # grid size (GRID x GRID cells)
T_MAX <- 200         # number of timesteps
P_BIRTH  <- 0.12
P_EAT    <- 0.10
P_DEATH  <- 0.06

# Initialize grid: each cell holds count of prey and predators
grid_state <- expand_grid(x = 1:GRID, y = 1:GRID) |>
  mutate(prey     = rbinom(n(), 2, 0.6),
         predator = rbinom(n(), 1, 0.15))

# Moore neighborhood function
moore_nbrs <- function(x, y, g = GRID) {
  expand_grid(dx = -1:1, dy = -1:1) |>
    filter(!(dx == 0 & dy == 0)) |>
    mutate(nx = ((x + dx - 1) %% g) + 1,
           ny = ((y + dy - 1) %% g) + 1) |>
    select(nx, ny)
}

# ... (full step function and animation loop in distributed materials)

# Animation call (requires gifski)
# anim <- ggplot(frames_df, aes(x = x, y = y, fill = prey_log)) + ...
#   transition_time(time) + ...
# anim_save("spatial_lv.gif", anim)
```
...
.center[
*[Pre-rendered animation: spatial_lv.gif]*

*Spiral wave formation in a 40×40 agent grid*

*Local interaction only — Moore neighborhood*
]
...
*All code at: [github.com/johnlynham/abm-statisticians]*
