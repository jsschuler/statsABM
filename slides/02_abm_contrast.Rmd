...
Block 1 derived two classical dynamical models from first principles and identified what is hiding in their structure.
...
ggplot() +
  ...
  labs(title    = "The ODE is the ABM's mean — not an approximation, a theorem",
...
As mixing becomes increasingly random and individual heterogeneity averages out, the ABM converges in distribution to the ODE. The ODE is what the ABM becomes when both restrictions hold exactly.
...
This dissolves the apparent competition between the two paradigms. There is no competition. The ABM is the richer language; the ODE is the sublanguage recovered under homogeneity and mean-field contact. The question is always whether the phenomena of interest are expressible in the sublanguage — and for many of the most consequential social and biological phenomena, they are not.
...
**Extinction.** Wolf overshoot drives the system to zero — an absorbing state the ODE cannot reach. The ODE orbits forever; the ABM can crash, and stay crashed.

???
Speaker notes: The image is the argument. The ODE with these parameters produces neutral cycles indefinitely. The ABM — same parameters, local interaction — produces boom-and-bust dynamics that terminate in extinction. The difference is not a quantitative deviation but a qualitative one: the absorbing state at zero is grammatically excluded from the ODE's language. Once populations hit zero in the ABM, there is no recovery. The ODE has no word for this.
...
The firm size distribution has been stable for decades, across countries with radically different institutions. Its form is a power law. No model that treats mean employment as the complete dynamical state can reproduce it. No ODE model of industry dynamics can generate it.
