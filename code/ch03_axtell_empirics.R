...
# Sections:
#   1. Setup
#   2. Load and visualize Axtell simulation firm size distribution
#   3. Power law fitting via MLE (poweRlaw package)
#   4. Log-normal comparison (Vuong test)
#
# Packages required:
#   tidyverse, poweRlaw, broom, patchwork
#
# Data note:
#   Chapter 3 uses firm size data from the Axtell (2001) ABM simulation,
#   parsed from Mathematica output into data/axtell/firm_size_distribution.csv
#   by code/parse_axtell_data.R. Run that script once before running this one.
#
#   Format: two columns — size (employment, integer) and count (number of firms).
#   Total: ~1.18 million firms; employment ranges from 1 to 152,843.
#
...
# The CSV was generated by code/parse_axtell_data.R from Mathematica output.
# Run parse_axtell_data.R once from the project root before running this script.
firm_size_dist <- read_csv("../data/axtell/firm_size_distribution.csv",
                           show_col_types = FALSE)
# firm_size_dist has two columns: size (integer employment) and count (number of firms).

# Expand aggregated counts to one row per firm.
# This is required by poweRlaw's displ$new(), which expects raw observations.
# Result: a tibble with ~1.18 million rows, one per firm.
...
# Set xmax to the observed maximum to avoid search-space truncation warning.
