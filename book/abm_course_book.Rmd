...
The spatial extension moves the Lotka-Volterra dynamics onto a discrete grid. Each cell of the grid can hold prey and predators. Interactions are local: agents interact only with their Moore neighborhood (the eight adjacent cells). Every individual's neighborhood is its eight immediate neighbors rather than the entire population.

The consequences are qualitative. The most striking is spiral waves of prey and predator density rotating through the grid — a dynamical regime with no name in the ODE's vocabulary, because it requires spatial coordinates to exist and the ODE has none. Before that: spatial structure sustains populations that the mean-field dynamics would drive to extinction, because local refugia can persist even when the global mean density would predict collapse.
